...
import { compressVideo, disposeIdleEngine, MAX_INPUT_BYTES, supportsMultithreading } from '../../compressor'
        ...
        <div className="upload-panel">
          ...
          {!file ? (
            ...
            <div className="selected-video">
              ...
              <div className="file-details"><FileVideo size={25} /><div><strong title={file.name}>{file.name}</strong><span>{size(file.size)} · Video original</span></div><button className="icon-button" disabled={busy} aria-label="Quitar video" onClick={() => { setFile(null); replaceURL('preview'); clearResult(); setError('') }}><X size={18} /></button></div>
            </div>
          )}
